...
class MonthlyStats:
    ...
    def _add_dollar_cpi_data_2monthly_stats(self):
        ...
        self.m_df = self.m_df.rename(columns={
                dcc.Dollar: (cte.empty_xl_col, dcc.Dollar),
                dcc.CPI   : (cte.empty_xl_col, dcc.CPI)})
        self._make_cols_multi_index()


    def _normalize_cpi(self):
        frst_month_cpi = self.m_df.iloc[0][(cte.empty_xl_col, dcc.CPI)]
        self.m_df[(cte.empty_xl_col, ms.normed_cpi)] = self.m_df[
                                                           (cte.empty_xl_col,
    ...
    def _cal_rev_in_real_dollar(self):
        self.m_df[(ms.rev_d, 'sum')] = self.m_df[(
                ms.rev_Hemmat, 'sum')] * 10 ** 3 / self.m_df[
                                           (cte.empty_xl_col, dcc.Dollar)]
        self.m_df[(ms.rev_r, 'sum')] = self.m_df[(
                ms.rev_Hemmat, 'sum')] / self.m_df[
                                           (cte.empty_xl_col, ms.normed_cpi)]
    ...
    def _find_base_yr_vals_4normalization(self):
        ...
        self.base_yr_vals = self.base_yr_vals.rename(columns=lambda x: f'{cte.base_year}_sum',
                                                     level=1)
    ...
    def _normalize_with_respect_to_base_yr(self):
        ...
        for ncol, nume in zip(nco, nume):
            self.m_df[(cte.empty_xl_col, ncol)] = self.m_df[(nume, 'sum')] / \
                                                  self.m_df[(
                                                          nume,
                                                          f'{cte.base_year}_sum')]
